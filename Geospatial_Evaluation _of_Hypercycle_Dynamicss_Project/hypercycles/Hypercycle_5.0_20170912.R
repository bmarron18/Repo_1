...
'''
https://en.wikipedia.org/wiki/Weibull_distribution
The Weibull distribution is the maximum entropy distribution for a non-negative 
real random variate when (1) the fixed expected value of xk is equal to λk a fixed 
expected value of ln(xk) equal to ln(λk) - \gamma.

k > 0 is the shape parameter and λ > 0 is the scale parameter

k ==> inf; Weibull ==> Dirac delta fxn
'''
...
#Explore Weibull dists and parameters ==> rweibull(n, shape, scale = 1)
library(Cairo)
...
# do 100 runs of the hypercycle model (one run for each unique param set) and send the output 
# of each run to its own file
# each run is a cell
for (i in 1:100) {
